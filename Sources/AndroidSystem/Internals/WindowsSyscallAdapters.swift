...
internal func pwrite(
    ...
    if !WriteFile(
        hFile, buf, DWORD(nbyte), &nNumberOfBytesWritten,
        &ovlOverlapped)
    {
        ucrt._set_errno(_mapWindowsErrorToErrno(GetLastError()))
        return Int(-1)
...
internal func mkdir(
    ...
    guard
        let pSD = _createSecurityDescriptor(
            from: actualMode,
            for: .directory)
    else {
        ucrt._set_errno(_mapWindowsErrorToErrno(GetLastError()))
    ...
    -> PSECURITY_DESCRIPTOR?
{
    ...
    let ownerDenyRights = ((ownerRights ^ groupRights) & groupRights) | ((ownerRights ^ otherRights) & otherRights)
    ...
    guard
        let pTokenUser = getTokenInformation(
            of: TOKEN_USER.self,
            hToken: hToken,
    ...
    guard
        let pTokenPrimaryGroup = getTokenInformation(
    ...
    guard
        !MakeSelfRelativeSD(&descriptor, nil, &dwRelativeSize)
